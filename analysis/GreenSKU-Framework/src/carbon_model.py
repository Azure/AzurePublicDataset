...
def get_dict_capex(components: Dict[str, Any], cost='cost') -> float:
...
def get_fan_power(base_server, base_fan, server_power, normalized=False):
    if normalized:
        norm_slope = 0.00127637
        slope = norm_slope * base_fan
    else:
        slope = 0.17869227
    power_diff = server_power - base_server
    return base_fan + slope * power_diff
...
class ServerCarbon:
    ...
    def get_rack_capacity(self) -> float:
    ...
    def _set_server_count(self) -> None:
        ...
        if 'num_servers' in self.config['rack'] and self.config['rack']['num_servers'] > 0 and self.config['rack']['num_servers'] is not None:
            ...
            if power_server_count < capacity_server_count:
                ...
                if self.print_out:
                    print(f"Power limited to {self.server_count} servers")
            else:
                self.power_limited = False
                self.server_count = capacity_server_count
                if self.print_out:
                    print(f"Space limited to {self.server_count} servers")
        if self.print_out:
    ...
    def _set_component_carbon(self) -> None:
        """Set the emissions for the server components."""
        for key, value in self.data.items():
            factor = self.params['PUE']
            factor *= self.params['power_factor']
            if key == 'rack' or key == 'server':
                total_opex = 0
            ...
            self.component_operational[key] = get_opex(value['power'],
                                                       ...
                                                       self.params['lifetime'],
                                                       factor=factor)
            self.component_embodied[key] = value['carbon'] * value['number']
    ...
    def get_rack_capex(self) -> float:
        """Get the capex for the rack."""
        return self.rack_capex
    
    def get_rack_sellable_cores(self) -> float:
        """Get the sellable cores for the rack."""
        return self.sellable_cores * self.socket_count * self.server_count

    def get_carbon_per_sellable_core(self) -> float:
        """Get the carbon per sellable core."""
        return round(self.get_rack_carbon() / (self.sellable_cores * self.socket_count * self.server_count), 2)
    
    def get_operational_per_sellable_core(self) -> float:
        """Get the operational per sellable core."""
        return round(self.get_rack_operational() / (self.sellable_cores * self.socket_count * self.server_count), 2)
    
    def get_embodied_per_sellable_core(self) -> float:
        """Get the embodied per sellable core."""
        return round(self.get_rack_embodied() / (self.sellable_cores * self.socket_count * self.server_count), 2)
    
    def get_capex_per_sellable_core(self) -> float:
        """Get the capex per sellable core."""
        return round(self.get_rack_capex() / self.get_rack_sellable_cores(), 2)
    
    def get_opex_per_sellable_core(self) -> float:
        """Get the opex per sellable core."""
        return round(self.get_rack_opex() / self.get_rack_sellable_cores(), 2)
    ...
    def _set_server_carbon(self) -> None:
        """Set the carbon emissions for the server."""
    ...
    def _set_rack_carbon(self) -> None:
        """Set the carbon emissions for the rack."""
        # set the carbon emissions for the components in the rack
        ...
        # set the total carbon emissions for the rack
